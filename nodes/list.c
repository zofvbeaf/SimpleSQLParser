...
list_concat(List *list1, List *list2)
{
	...
	if (list2 == NIL)
		return list1;
	if (list1 == list2)
		fprintf(stderr, "cannot list_concat() a list to itself");
 ...
 * list2.  The returned list is freshly allocated via malloc(), but the
...
list_qsort(const List *list, list_qsort_comparator cmp)
{
	...
	list_arr = (ListCell **) malloc(sizeof(ListCell *) * len);
	i = 0;
	foreach(cell, list)
		list_arr[i++] = cell;
